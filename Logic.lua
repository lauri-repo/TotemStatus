...
--[[
	This is called when settings are changed and will set timer color to white to make 
	sure expiry alarm setting change is properly applied.
	Otherwise the timer could possibly get stuck with red text.
]]
...
function TS.Logic.CombatLogEvent()
	...
	if (eventName == "SPELL_CAST_SUCCESS" and j == "Healing Wave" and playerName == UnitName("player"))
	then
		print(procs / casts)
		casts = casts + 1
	end
	if (eventName == "SPELL_ENERGIZE" and playerName == UnitName("player"))
	then
		procs = procs + 1
	end
